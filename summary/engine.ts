  ...
  return [
    {
      ...
      content: [
        ...
        '  着装.部位:描述（例：着装.上装:换上了黑色卫衣）',
        '  当前事件:事件ID（设置当前主线事件；清空用 当前事件:无）',
        '  主线事件.事件ID:状态（未触发/进行中/已结束/跳过/延后）',
        '  事件:本轮剧情=简短概括',
        '  物品+名称:数量:描述（获得物品；例：物品+蛋包饭券:1:英梨梨要求的交换条件）',
        '  物品-名称（失去或使用物品；例：物品-塑料袋）',
        '<state_delta> 规则：默认变量目标不是当前镜头焦点；只更新正文里明确出现或直接受影响的角色。多角色同时在场时，可以分别输出好感度。不要把某个角色的审计或关系规则套给其他角色。',
        '<state_delta> 低好感加藤惠校准：普通、礼貌、有边界的日常互动也可以给 +1；不要因为她反应平淡就误判为无变化或负反馈。',
        '示例：',
        '<summary>',
