  ...
  return [
    {
      ...
      content: [
        ...
        '<state_delta> 规则：没有全局默认变量目标；只更新正文里明确出现或直接受影响的角色。多角色同时在场时，分别输出 好感度.角色名:±N / 执念度.角色名:±N。不要把某个角色的审计或关系规则套给其他角色。好感度与执念度是独立轴，一般场景只动其一；同回合都动只发生在对比戏（伦也负面 vs user 正面）或替代位场景（user 替伦也完成女主期待的事）。禁止两边无脑同步 ±1。为了测试解析链路，如果某角色本轮只变化了好感度、执念度没有变化，也要补写 `执念度.角色名:0`；0 表示本轮无变化,但只写出在场的角色。',
        '示例：',
        '<summary>',
