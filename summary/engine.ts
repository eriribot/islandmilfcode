...
const MINOR_SUMMARY_FRAMEWORK = [
  ...
  '职责边界：小总结只总结本次窗口内真实出现的内容；旧摘要、旧事件、角色卡和世界书不参与本段事实判定。',
  '处理步骤：',
  ...
  '5. 保留能影响下一轮续写的细节：谁说了什么要点、谁做了什么、交换条件、未完成动作、角色反应、物品/手机/地点线索。',
  '6. 最后抽取 key_facts；没有长期价值就输出空标签。',
  '输出期望：summary 约 180~260 字；细节密集时可到 320 字。重点是事实连续性，不写文学润色。',
].join('\n');
...
/** 构建小摘要 prompt：对一段对话片段做剧情记录，同时抽取结构化关键事实。 */
  ...
  return [
    {
      ...
      content: [
        ...
        '- 不要忽略当前窗口内的具体细节：对话要点、动作结果、情绪反应、交换条件、未解决问题、物品/手机/地点线索。',
        '- 禁止只写“两人聊天/讨论某事/关系推进”这类空泛句；必须点出具体讨论对象、决定和结果。',
        '',
        ...
        '五轮里发生了请求、拒绝、交换条件和情绪反应，却只写“User 和英梨梨继续聊天”。正确写法是保留请求内容、拒绝理由、交换条件和结果。',
        anchorBlock,
      ]
        .filter(Boolean)
        .join('\n'),
