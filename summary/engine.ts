  ...
  return [
    {
      ...
      content: [
        ...
        '  执念度.角色名:±N（仅限白名单：加藤惠 / 英梨梨 / 霞之丘诗羽 / 波岛出海 / 美智留。其他角色（红坂朱音、丸户、伦也本人、任何 NPC）禁止输出执念度字段。',
        '    角色对伦也旧线的牵挂变化，与好感度独立。允许扣减的通道：',
        ...
        '<state_delta> 规则：没有全局默认变量目标；只更新正文里明确出现或直接受影响的角色。多角色同时在场时，分别输出 好感度.角色名:±N。执念度只针对白名单（加藤惠 / 英梨梨 / 霞之丘诗羽 / 波岛出海 / 美智留），其他角色不写。不要把某个角色的审计或关系规则套给其他角色。好感度与执念度是独立轴，一般场景只动其一；同回合都动只发生在对比戏（伦也负面 vs user 正面）或替代位场景（user 替伦也完成女主期待的事）。禁止两边无脑同步 ±1。补 0 规则：仅在白名单角色本轮只动好感度、执念度未变化时，才补写 `执念度.角色名:0` 用于解析链路体检；非白名单角色一律不写执念度字段（包括 0）。',
        '示例：',
        '<summary>',
        ...
        '</state_delta>',
        '（注：以上是日常戏示例，只动好感度，执念度未变化要写 0表示。若同回合出现伦也负面 + user 正面的对比戏，可同时输出 好感度.英梨梨:+3 与 执念度.英梨梨:-5；若伦也未出场但 user 替伦也陪她改稿（替代位），可同时输出 好感度.英梨梨:+2 与 执念度.英梨梨:-2。）',
        anchorBlock,
      ]
        .filter(Boolean)
        .join('\n'),
  ...
  return [
    {
      ...
      content: [
        ...
        '</summary>',
        '',
        '【可选】<state_delta> 标签：仅当压缩过程中浮现了"小摘要尚未抓住的跨段状态变化"才输出，没有就输出空标签 <state_delta></state_delta>。',
        '允许场景：(a) 多段 minor 累积才看清的关系阶段升级；(b) 跨 minor 的长伏笔在本次压缩里回收；(c) 长跨度才能判断的角色情绪整体偏移。',
        '禁止场景：把已经在 minor state_delta 里写过的变化再写一遍（会被双重消费）；逐段重新解读 minor 已涵盖的剧情。',
        '字段语义和 minor 的 <state_delta> 一致（增量 ±N 而非快照）。可用字段示例：',
        '  好感度.角色名:±N（执念度仅限：加藤惠 / 英梨梨 / 霞之丘诗羽 / 波岛出海 / 美智留）',
        '  执念度.角色名:±N',
        '  主线事件.事件ID:状态',
        '  当前事件:事件ID',
        '99% 的轮次输出 <state_delta></state_delta> 即可；少量真正跨段的状态变化才填具体字段。',
        anchorBlock,
        pinnedBlock,
      ]
        .filter(Boolean)
  ...
  return [
    {
      ...
      content: [
        ...
        '</summary>',
        '',
        '【可选】<state_delta> 标签：仅当全局压缩过程中浮现"大摘要尚未抓住的跨段状态变化"才输出，没有就输出空标签 <state_delta></state_delta>。',
        '允许场景：跨多段 major 才看清的整体关系拐点、被压缩遮蔽的累积情绪偏移、长伏笔回收。',
        '禁止场景：重写已经被 minor/major state_delta 消费过的变化（会被双重计入）。',
        '字段语义和 minor 的 <state_delta> 一致（增量 ±N 而非快照）。可用字段示例：',
        '  好感度.角色名:±N（执念度仅限：加藤惠 / 英梨梨 / 霞之丘诗羽 / 波岛出海 / 美智留）',
        '  执念度.角色名:±N',
        '  主线事件.事件ID:状态',
        '  当前事件:事件ID',
        '99% 的轮次输出 <state_delta></state_delta> 即可。',
        anchorBlock,
        pinnedBlock,
      ]
        .filter(Boolean)
