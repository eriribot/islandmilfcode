  ...
  return [
    {
      ...
      content: [
        ...
        '  贞操.角色名:已失去（仅当正文明确发生破除时输出，或状态快照显示完璧但对话上下文中已明确发生过性行为时必须补写修正；无好感/旧情门槛，只看正文是否真的发生。单向不可逆，禁止写"完璧/处女"复位）',
        '  X次数.角色名:+N（亲密接触硬统计，X 为开放字段：接吻次数/口交次数/乳交次数/性交次数/被内射次数/肛交次数，特殊玩法可自定义如 足交次数；正文明确发生对应行为时 +N，或状态快照计数与对话上下文已知事实不符时一次性补正。不统计经验人数/伴侣数）',
