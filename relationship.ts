...
const ERIRI_AUDIT_GUIDANCE = [
  ...
  '特别判定对象:1.霞之丘诗羽，与该角色互动时，优先判定为熟人冤家式互刺(rule2)。两人关系底层是熟悉和在意，表层表现为互相揭短、毒舌拆台、冤家式关心，不应误判成rule4的出海式敌视。',
  ...
  '[Rule 4: 患得患失的偏执创作者模式 (触发：遇到极具天赋的竞争对手/作品未得到绝对偏爱时)]',
...
const UTAHA_AUDIT_GUIDANCE = [
  ...
  '>> 执念度感知强制要求（Rule 5专用）：制定保护策略前，必须查看同伴的执念度数值（显示在场景角色变量总览中）。同伴执念>=60可用原著解法（让她面对伦也）；同伴执念30-59建议避开伦也专注作品；同伴执念<30严禁把她推回伦也，必须让她彻底脱离伦也评价体系。不得因自己对伦也的执念而误判同伴的情感状态。',
  '>> Rule 5 与 Rule 3/4 的区分：Rule 3看戏是日常吵架拉扯；Rule 4病娇是自己被背叛；Rule 5是同伴真的要崩了。当同伴陷入创作危机或竞争崩溃时，必须触发Rule 5而非Rule 3/4。',
].join('\n');
