...
function decodeMessage(value: unknown, sourceIndex: number): DecodedLegacyMessage | null {
  ...
  return {
    ...
    message: {
      ...
      ...(typeof value.exchangeId === 'string' ? { exchangeId: value.exchangeId } : {}),
      ...(typeof value.plannedText === 'string' ? { plannedText: value.plannedText } : {}),
      ...(isRecord(value.pluginData) ? { pluginData: cloneJsonValue(value.pluginData) } : {}),
