...
export function buildPrompt(
  ...
  const saenaiDialogueInstruction = [
    ...
    '如果可见正文里出现以下八名角色的面对面台词，必须在该台词同一行最前面加 @saenai:角色名。',
    '八名角色：加藤惠、英梨梨、霞之丘诗羽、波岛出海、冰堂美智留、泽村小百合、町田苑子、红坂朱音。',
    '角色名必须原样复制这八个名字之一；不要写泽村英梨梨/加藤恵/霞诗子/高坂茜等别名。',
    '安艺伦也、玩家、旁白、手机消息和名单外角色禁止使用 @saenai。',
    '禁止输出未带 @saenai 的八名角色独立引号对白。凡是这八名角色说出的「台词」，都必须写成 @saenai:角色名「台词」。',
    ...
    '正确示例：',
    '@saenai:红坂朱音「有趣。你们终于像个团队了。」',
  ].join('\n');

  const parts = [
    '你正在执行当前轮正文生成：把本轮已经筛选好的剧情信息、角色信息和玩家输入落实为自然正文，不要复述设定、重做审稿或整理摘要。',
...
function buildProgressInstruction(statusData: StatusData, target?: TargetStatus | null): string {
  ...
  return [
    ...
    `执念度专指角色对伦也这条旧线的牵引强度；只对 ${OBSESSION_TARGET_DISPLAY_NAMES} 五人有效，不要把它当成对 User 的关系温度。其他在场角色（包括红坂朱音、町田苑子、西宫硝子、泽村小百合、丸户、伦也本人或任何 NPC）一律不输出执念度字段；其中红坂朱音/町田苑子/西宫硝子仍可按在场互动更新好感度。`,
    '亲密/背德轴判断规则：',
    '  默认角色使用“贞操”闩锁：只有角色列表显示“贞操=完璧”时，才允许在明确发生破除后输出 贞操.角色名:已失去。',
    '  泽村小百合是特殊的既婚人妻档案，不使用“贞操/完璧/处女”语义，也不输出贞操字段。町田苑子、红坂朱音虽为成人未婚角色，但按默认“完璧/结缘”闩锁处理，可输出贞操字段。',
    ...
    `  执念度.角色名或id:±N    — 仅限白名单（${OBSESSION_TARGET_DISPLAY_NAMES}），语义是角色对伦也旧线/锚点的牵引（例：${obsessionExamples}）。名单外角色（包括红坂朱音、泽村小百合、町田苑子、西宫硝子、伦也本人）禁止输出；红坂朱音/町田苑子/西宫硝子只能输出好感度，不输出执念度。`,
    ...
    target
      ? hasObsessionAxis(target)
        ...
        : `  当前明确变量对象: ${target.name}；好感度: ${target.affinity} (${target.stage})；无旧情/执念轴`
...
function buildStateDeltaInstruction(statusData: StatusData): string {
  ...
  return [
    ...
    `执念度只允许写 ${OBSESSION_TARGET_DISPLAY_NAMES}；红坂朱音、町田苑子、西宫硝子、泽村小百合、伦也和其他 NPC 禁止输出执念度字段。`,
    '五维.能力名:±N',
  ...
  return [
    {
      ...
      content: [
        ...
        '  普通友好互动或者逗乐大家的行动通常 +1；明显关心、理解、协助、保护通常 +2 到 +4；冒犯、越界、揭短、冷落通常 -1 到 -6。',
        '  生活托底：当角色处于现实失稳或重大压力中，User 承担其住处、金钱、家庭/学业/事业缓冲、风险兜底等基础支持，并使角色产生安全感、亏欠感或稳定依赖时，通常 +6 到 +10；若本轮明确体现角色已把 User 当成关键依靠，优先 +8 左右。',
        '  不要把生活托底误判为普通关心；只有没有实际托底、没有风险或责任承担、没有明显安全感/依赖变化时，才按日常互动处理。',
        '  不要因为变化很小就省略好感度；只有角色不在场、完全无互动、纯环境描写、或关系没有任何变化时，才不输出该角色好感度。',
        '执念度判断规则（与好感度独立的对伦也旧情度，对伦也旧线的牵挂,对伦也的好感度）：',
        `  作用范围：仅限 ${OBSESSION_TARGET_DISPLAY_NAMES} 五人。其他角色（包括红坂朱音、町田苑子、西宫硝子、泽村小百合、丸户、伦也本人或任何 NPC）禁止输出执念度字段；红坂朱音/町田苑子/西宫硝子/泽村小百合若在场并对 User 有反应，只评估好感度。`,
        ...
        options?.includePhoneMessages
          ? [
              ...
              '  6. 非攻略对象（伦也、丸户等）发来的消息不要输出；红坂朱音、町田苑子、西宫硝子属于可攻略特别/DLC档案，若在可更新角色列表中出现，可以落到对应 thread。',
