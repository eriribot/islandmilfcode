...
function buildProgressInstruction(statusData: StatusData, target?: TargetStatus | null): string {
  ...
  return [
    ...
    `执念度专指角色对伦也这条旧线的牵引强度；只对 ${OBSESSION_TARGET_DISPLAY_NAMES} 五人有效，不要把它当成对 User 的关系温度。其他在场角色（包括红坂朱音、町田苑子、泽村小百合、丸户、伦也本人或任何 NPC）一律不输出执念度字段；其中红坂朱音/町田苑子仍可按在场互动更新好感度。`,
    '亲密/背德轴判断规则：',
    ...
    `  执念度.角色名或id:±N    — 仅限白名单（${OBSESSION_TARGET_DISPLAY_NAMES}），语义是角色对伦也旧线/锚点的牵引（例：${obsessionExamples}）。名单外角色（包括红坂朱音、泽村小百合、町田苑子、伦也本人）禁止输出；红坂朱音/町田苑子只能输出好感度，不输出执念度。`,
  ...
  return [
    {
      ...
      content: [
        ...
        `  作用范围：仅限 ${OBSESSION_TARGET_DISPLAY_NAMES} 五人。其他角色（包括红坂朱音、町田苑子、泽村小百合、丸户、伦也本人或任何 NPC）禁止输出执念度字段；红坂朱音/町田苑子/泽村小百合若在场并对 User 有反应，只评估好感度。`,
        ...
        options?.includePhoneMessages
          ? [
              ...
              '  6. 非攻略对象（伦也、丸户等）发来的消息不要输出；红坂朱音、町田苑子属于可攻略特别档案，若在可更新角色列表中出现，可以落到对应 thread。',
